  ...
  <div class="register">
    <div class="signUp">
      ...
      <div class="input-group" style="border-radius: 25px;">
        <button class="form-control"style="border-radius: 25px;" @click="register()">注册</button>
        <p class="goLogin">已有账号？<a href="http://localhost:3000/login">去登录</a></p>
      </div>
    </div>
  </div>
</template>
  ...
  methods: {
    register () {
      ...
      axios.post('/api/register', {
        ...
        this.isRegisterSuccess.prompt = ['此账号已注册', '注册成功', '注册失败'][response.data.code]
